...
async function generateProducts(niche) {
  ...
  prompt += "SEO-titler maks 140 tegn, pris $4.99-$19.99, 13 tags maks 20 tegn. ";
  prompt += "Svar KUN med JSON (ingen markdown): ";
  prompt += "{\"products\":[{\"title\":\"T\",\"description\":\"100 ord\",\"price\":9.99,";
  prompt += "\"tags\":[\"t1\",\"t2\",\"t3\",\"t4\",\"t5\",\"t6\",\"t7\",\"t8\",\"t9\",\"t10\",\"t11\",\"t12\",\"t13\"],";
  prompt += "\"category\":\"Digital Downloads\",\"type\":\"digital\",\"imageKeyword\":\"keyword\",";
  prompt += "\"pinterest_caption\":\"pin tekst #hashtag\",\"instagram_caption\":\"ig tekst\",\"tiktok_hook\":\"hook\"}]}";
...
async function checkEtsyPerformance() {
  ...
  if (!CFG.ETSY_API_KEY || !CFG.ETSY_SHOP_ID) {
    log("Etsy-nokler mangler - hopper over ytelsessjekk", "warn");
    return;
  }
  try {
    var res = await axios({
    ...
    listings.forEach(function(listing) {
      ...
      if (prod) {
        prod.views     = listing.views || 0;
        prod.favorites = listing.num_favorers || 0;
        if (prod.niche) trackNiche(prod.niche, "views", listing.views || 0);
      }
    });
...
function getImageUrl(keyword) {
  var colors = { planner:"1a1a2e/B8FF47", wedding:"2d1b69/FFD6E7", svg:"0f3460/00E87A",
    baby:"533483/FFE4B5", dog:"1e3a5f/FFA500", budget:"0d2137/00D4FF",
    recipe:"3d1a00/FF8C42", fitness:"1a3a1a/B8FF47" };
  var key = Object.keys(colors).find(function(k) { return (keyword || "").toLowerCase().includes(k); }) || "planner";
  var parts = colors[key].split("/");
  var label = encodeURIComponent((keyword || "Product").slice(0, 20));
  return "https://via.placeholder.com/600x900/" + parts[0] + "/" + parts[1] + "?text=" + label;
}

async function listOnEtsy(product) {
  if (!CFG.ETSY_API_KEY || !CFG.ETSY_SHOP_ID) {
    log("Etsy-nokler mangler - simulerer lansering (MOCK)", "warn");
...
async function pinOnPinterest(product, etsyUrl) {
  if (!CFG.PINTEREST_TOKEN || !CFG.PINTEREST_BOARD) {
    log("Pinterest-token mangler - simulerer pin (MOCK)", "warn");
...
async function runAgentCycle() {
  ...
  try {
    ...
    log(products.length + " produkter generert for: " + niche);
    ...
    for (var i = 0; i < products.length; i++) {
      ...
      try {
        ...
        state.queue.push({
          ...
          pinterest_caption: product.pinterest_caption,
          platforms:         ["instagram", "tiktok"],
          queuedAt:          new Date().toISOString(),
        });
        state.stats.pendingApproval = state.queue.length;
        log("Lagt i godkjenningskoeen for Instagram/TikTok", "info");

      } catch (err) {
        log("Feil ved lansering av " + (product.title || "").slice(0, 30) + ": " + err.message, "error");
      }
...
app.get("/status", function(req, res) {
  res.json({ running: state.running, stats: state.stats, lastCycle: state.lastCycle,
    nextCycleAt: state.nextCycleAt, lastStrategy: state.lastStrategy,
    lastPerformanceCheck: state.lastPerformanceCheck, activeNiches: state.activeNiches });
});
...
app.post("/start", function(req, res) {
  state.running = true;
  log("Agent startet! 24/7 modus aktivert.", "success");
  runAgentCycle();
  res.json({ ok: true });
});
